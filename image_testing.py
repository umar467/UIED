...
def optical_change():
    ...
    out_path = 'data/output/frames/11_frame_averaged/'
    in_path = "data/input/videos/11.mp4"
    
    Path(out_path).mkdir(parents=True, exist_ok=True)
    camera = cv2.VideoCapture(in_path)
    ...
    while (keep_reading):
     ...
     if frame_averaging:
         drawn = draw.draw_bounding_box(frame, uicompos, show=True, name='components', wait_key=1, fg =fg)
         fg_avg = fg_avg + draw.avgboxx(frame, uicompos)
     else:
         drawn = draw.draw_bounding_box(frame, uicompos, show=True, name='components', wait_key=1)
     #cv2.imwrite(out_path+str(fno)+'.jpg',drawn)
